...
void biosc_main(void);
void biosc_datacap(void);
...
)
{
	...
	&&  pixel_measure_mode != MD_MEASURE_MODE_BIOSC
        )
    {
...
void biosc_main(){
	...
	md_init_motion_detector(9,9,3,10,3,5,3,1,3,3,7,7,0,2,0);
	//char* strloc = "Sucesss";
	//sprintf(strloc, "main entered");
	//
	int fp = open("A/rawsigs.txt", O_RDWR|O_CREAT , 0777);
	//sprintf(stdout, "main entered");
	sprintf(wrbuf, "[");
	biosc_datacap();
	for(i = 0; i < 9; i++){
		for(j = 0; j < 9; j++){
			sprintf(wrbuf, "%d", val[i][j]);
		}
	}
	sprintf(wrbuf, "]");
	write(fp, wrbuf, 100);
...
void biosc_datacap(void){
int i, j, zone;
zone = md_detect_motion();
for(i = 0; i < 9; i++){
	for(j = 0; j < 9; j++){
		val[i][j] = md_get_cell_val(i, j);
	}
}
